...
// enable pin setting for object/shape detection
#define CONFIG_PINS_ENABLED 1


// Pin setting for object/shape detection
#if CONFIG_PINS_ENABLED


#define CONFIG_PIN_GPIO_14 1
#define CONFIG_PIN_GPIO_13 1
// #define CONFIG_PIN_GPIO_12 1
#define PinToRead 15

// As Input PIN
// #define CONFIG_PIN_GPIO_16 1 

// Global GPIO state variables
int gpio_14 = 0;
int gpio_13 = 0;
// int gpio_12 = 0;
...
// #if CONFIG_PINS_ENABLED
// void ControlPin(int gpio_num, int level) {
...
static esp_err_t gpio_read_handler(httpd_req_t *req)
{ 
    // const int pinToRead = PinToRead;


    int state = gpio_get_level((gpio_num_t)PinToRead);

    // Build a JSON string, or plain text if you prefer
    char response[64];
    snprintf(response, sizeof(response), "{\"gpio_state\": %d}", state);

    // Set the content type to JSON (or text/plain if that’s what you want)
...
#endif
#if CONFIG_PIN_GPIO_14 && CONFIG_PIN_GPIO_13
else if (!strcmp(variable, "gpio_14")) {
    gpio_14 = val;
    log_i("Received request: gpio_14 = %d", gpio_14); // Debug log
    // gpio_set_level((gpio_num_t)14, gpio_14);
    digitalWrite(14, gpio_14);

    log_i("GPIO 14 set to: %d", gpio_get_level((gpio_num_t)14)); // Confirm it's actually set
}

else if (!strcmp(variable, "gpio_13")) {
  gpio_13 = val;
  log_i("Received request: gpio_13 = %d", gpio_13); // Debug log
  digitalWrite(13, gpio_13);

  // gpio_set_level((gpio_num_t)13, gpio_13);
  log_i("GPIO 13 set to: %d", gpio_get_level((gpio_num_t)13)); // Confirm it's actually set

}
#endif
// #if CONFIG_PIN_GPIO_12
//   else if (!strcmp(variable, "gpio_12")) {
//   gpio_12 = val;
//   gpio_set_level((gpio_num_t)12, gpio_12);
// }
// #endif
...
#endif
#if CONFIG_PIN_GPIO_14
  p += sprintf(p, ",\"gpio_14\":%u", gpio_14);
#endif
// #if CONFIG_PIN_GPIO_12
//   p += sprintf(p, ",\"gpio_12\":%u", gpio_12);
// #endif
#if CONFIG_PIN_GPIO_13
  p += sprintf(p, ",\"gpio_13\":%u", gpio_13);
#endif
#if CONFIG_ESP_FACE_DETECT_ENABLED
...
void startCameraServer() {
  ...
  // config.stack_size = 8192;  // Increase stack size from 4096 to 8192
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &gpio_read_uri);
...
#if CONFIG_PINS_ENABLED
void setup3pins(int pin1, int pin2, int pin3) {
    // Initialize GPIO pin(s) as in/outputs
    pinMode(pin1, OUTPUT);
    pinMode(pin2, OUTPUT);
